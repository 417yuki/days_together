...
describe("unknown sprout consultation", () => {
  ...
  it("rejects missing, multiple, array, and oversized candidates", () => { expect(() => extractJsonObject("ありません")).toThrow(); expect(() => extractJsonObject("{} と {}" )).toThrow("複数"); expect(() => extractJsonObject("[1,2]" )).toThrow(); expect(() => extractJsonObject("x".repeat(MAX_RESPONSE_LENGTH + 1))).toThrow("20,000"); });
