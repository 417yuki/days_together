...
describe("custom map finish", () => {
  ...
  const drafts = { starter_house_interior: parseCustomMapDraft({ ...valid("starter_house_interior"), locations: [location("inside")] })!, starter_garden: parseCustomMapDraft({ ...valid("starter_garden"), locations: [location("outside")] })! };
  const gateways = { starter_house_interior: { gatewayLocationId: "inside", proxyPosition: { x: .2, y: .3 }, entryAffordancePosition: null }, starter_garden: { gatewayLocationId: "outside", proxyPosition: { x: .8, y: .7 }, entryAffordancePosition: { x: .5, y: .4 } } };
  ...
  it("restores a valid gateway pair and removes duplicate items in fixed order", () => { const result = restoreCustomMapFinishPair(drafts, gateways, slots, ["tea"]); expect(result.starter_house_interior.gatewayVisual.gatewayLocationId).toBe("inside"); expect(result.starter_garden.gatewayVisual.gatewayLocationId).toBe("outside"); expect(result.starter_house_interior.itemSlots.interior_slot_1.itemId).toBe("tea"); expect(result.starter_garden.itemSlots.garden_slot_1.itemId).toBeNull(); });
  it("repairs both gateway ids and coordinate components independently", () => { const broken = { starter_house_interior: { ...drafts.starter_house_interior, gatewayVisual: { gatewayLocationId: "missing", proxyPosition: { x: Number.NaN, y: .44 }, entryAffordancePosition: { x: 1, y: 1 } } }, starter_garden: { ...drafts.starter_garden, gatewayVisual: { gatewayLocationId: "outside", proxyPosition: { x: .6, y: .7 }, entryAffordancePosition: { x: Number.NaN, y: .66 } } } }; const result = restoreCustomMapFinishPair(broken, gateways, slots, ["tea"]); expect([result.starter_house_interior.gatewayVisual.gatewayLocationId, result.starter_garden.gatewayVisual.gatewayLocationId]).toEqual(["inside", "outside"]); expect(result.starter_house_interior.gatewayVisual.proxyPosition).toEqual({ x: .2, y: .44 }); expect(result.starter_garden.gatewayVisual.entryAffordancePosition).toEqual({ x: .5, y: .66 }); expect(result.starter_house_interior.gatewayVisual.entryAffordancePosition).toBeNull(); });
});
