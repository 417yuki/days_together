...
export const parseItem = (value: unknown): GameItem | null => {
  ...
  if (typeof v.itemId !== "string" || v.itemId.length < 1 || v.itemId.length > 100 || control.test(v.itemId) || typeof v.name !== "string" || v.name !== v.name.trim() || v.name.length < 1 || v.name.length > 40 || control.test(v.name) || !itemCategories.includes(v.category as ItemCategory) || typeof v.description !== "string" || v.description !== v.description.trim() || v.description.length > 280 || control.test(v.description) || !Array.isArray(v.tags) || v.tags.length > 12 || !v.tags.every((tag) => typeof tag === "string" && tag === tag.trim() && tag.length >= 1 && tag.length <= 24 && !control.test(tag)) || new Set(v.tags).size !== v.tags.length || !["preset", "user", "event"].includes(v.source as string) || !visual || typeof visual.iconKey !== "string" || visual.iconKey.length < 1 || !(visual.genericVisualKey === null || typeof visual.genericVisualKey === "string") || visual.mapDisplayMode !== "icon" || typeof v.createdAt !== "string" || !Number.isFinite(Date.parse(v.createdAt)) || typeof v.updatedAt !== "string" || !Number.isFinite(Date.parse(v.updatedAt))) return null;
  return { itemId: v.itemId, name: v.name, category: v.category as ItemCategory, description: v.description, tags: [...v.tags] as string[], source: v.source as ItemSource, visual: { iconKey: visual.iconKey, genericVisualKey: visual.genericVisualKey as string | null, imageAssetId: isSafeAssetId(visual.imageAssetId) ? visual.imageAssetId : null, mapDisplayMode: "icon" }, createdAt: v.createdAt, updatedAt: v.updatedAt };
};
