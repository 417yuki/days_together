import { ACTION_DEFINITIONS, ACTION_LINES, actionIds, type ActionId } from "./partnerActions";

export const partnerTraitIds = ["initiative", "curiosity", "sociability", "caretaking", "affection", "tidiness", "patience", "moodVolatility", "solitudePreference", "userPriority", "adventurousness", "caution"] as const;
export type PartnerTraitId = typeof partnerTraitIds[number];
export type PartnerGameProfile = { profileId: "main_partner"; characterId: "cody"; revision: number; source: "preset" | "manual_setup" | "manual_update" | "history_restore"; displayName: string; traits: Record<PartnerTraitId, number>; preferredActionIds: ActionId[]; dislikedActionIds: ActionId[]; createdAt: string; updatedAt: string };
export type PartnerDialogueLine = { dialogueId: string; profileId: "main_partner"; actionId: ActionId; text: string; enabled: boolean; createdAt: string; disabledAt: string | null; sourceRevision: number };
export type PartnerProfileSnapshot = { profile: PartnerGameProfile; dialogues: PartnerDialogueLine[] };
export type PendingPartnerConsultation = { requestId: string; requestType: "partner_profile_setup" | "partner_profile_update"; profileId: "main_partner"; expectedRevision: number; prompt: string; createdAt: string; status: "pending" };
export type PartnerSetupResult = { schemaVersion: 1; requestId: string; requestType: "partner_profile_setup"; profileId: "main_partner"; displayName: string; traits: Record<PartnerTraitId, number>; preferredActionIds: ActionId[]; dislikedActionIds: ActionId[]; dialogues: Array<{ actionId: ActionId; text: string }> };
export type PartnerUpdateResult = { schemaVersion: 1; requestId: string; requestType: "partner_profile_update"; profileId: "main_partner"; expectedRevision: number; displayName: string | null; traitUpdates: Partial<Record<PartnerTraitId, number>>; preferredActionIds: ActionId[] | null; dislikedActionIds: ActionId[] | null; addDialogues: Array<{ actionId: ActionId; text: string }>; disableDialogueIds: string[] };
export type PartnerResult = PartnerSetupResult | PartnerUpdateResult;

const presetTime = "2026-07-28T00:00:00.000Z";
export const codyPresetProfile: PartnerGameProfile = { profileId: "main_partner", characterId: "cody", revision: 1, source: "preset", displayName: "コーディ", traits: { initiative: 72, curiosity: 82, sociability: 58, caretaking: 78, affection: 64, tidiness: 70, patience: 76, moodVolatility: 30, solitudePreference: 47, userPriority: 80, adventurousness: 56, caution: 62 }, preferredActionIds: ["cook", "garden", "craft", "inspect_item"], dislikedActionIds: [], createdAt: presetTime, updatedAt: presetTime };
