...
describe("Store", () => {
  ...
  it("announces the current and final locations while moving, then the arrival", () => { const store = new Store(); store.beginMovement("user", { mapId: "starter_garden", locationId: "garden" }); expect(store.getState().message).toContain("テーブルにいて、庭へ移動中"); while (store.advanceCharacter("user")) { expect(store.getState().message).toMatch(/にいて、庭へ移動中/); } expect(store.getState().message).toContain("庭に着きました"); });
});
