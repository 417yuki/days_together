...
import { normalizeCustomLocations, normalizeCustomMapName, parseCustomMapDraft, restoreCustomMapFinishPair, type CustomMapDraft, type CustomMapFinishPair, type CustomMapLocation, type CustomMapTargetId } from "../domain/maps/customMapDraft";
...
export class IndexedDbSaveRepository implements SaveRepository {
  ...
  async putCustomMapDraft(targetMapId: CustomMapTargetId, name: string, now: string): Promise<CustomMapDraft> { const db = await this.open(); const tx = db.transaction(STORE_NAMES.customMapDrafts, "readwrite"); const store = tx.objectStore(STORE_NAMES.customMapDrafts); const existing = parseCustomMapDraft(await request(store.get([MAIN_SAVE_SLOT_ID, targetMapId])), targetMapId); const draft: CustomMapDraft = { ...existing, saveSlotId: MAIN_SAVE_SLOT_ID, targetMapId, name: normalizeCustomMapName(name, targetMapId), status: "draft", locations: existing?.locations ?? [], createdAt: existing?.createdAt ?? now, updatedAt: now }; store.put(draft); await transactionDone(tx); return draft; }
  async putCustomMapLocations(targetMapId: CustomMapTargetId, locations: CustomMapLocation[], now: string): Promise<CustomMapDraft> { const db = await this.open(); const tx = db.transaction(STORE_NAMES.customMapDrafts, "readwrite"); const store = tx.objectStore(STORE_NAMES.customMapDrafts); const existing = parseCustomMapDraft(await request(store.get([MAIN_SAVE_SLOT_ID, targetMapId])), targetMapId); if (!existing) { tx.abort(); throw new Error("下書きがありません"); } const normalized = normalizeCustomLocations(locations); if (normalized.length < 1) { tx.abort(); throw new Error("地点は一つ以上必要です"); } const draft = { ...existing, locations: normalized, updatedAt: now }; store.put(draft); await transactionDone(tx); return draft; }
  async putCustomMapFinishPair(pair: CustomMapFinishPair, itemIds: string[], now: string): Promise<Record<CustomMapTargetId, CustomMapDraft>> { const db = await this.open(); const tx = db.transaction(STORE_NAMES.customMapDrafts, "readwrite"); const store = tx.objectStore(STORE_NAMES.customMapDrafts); const interior = parseCustomMapDraft(await request(store.get([MAIN_SAVE_SLOT_ID, "starter_house_interior"])), "starter_house_interior"), garden = parseCustomMapDraft(await request(store.get([MAIN_SAVE_SLOT_ID, "starter_garden"])), "starter_garden"); if (!interior?.locations.length || !garden?.locations.length) { tx.abort(); throw new Error("両方の地点設定が必要です"); } const clean = restoreCustomMapFinishPair({ starter_house_interior: { ...interior, ...pair.starter_house_interior }, starter_garden: { ...garden, ...pair.starter_garden } }, { starter_house_interior: pair.starter_house_interior.gatewayVisual, starter_garden: pair.starter_garden.gatewayVisual }, { starter_house_interior: pair.starter_house_interior.itemSlots, starter_garden: pair.starter_garden.itemSlots }, itemIds); const result = { starter_house_interior: { ...interior, ...clean.starter_house_interior, updatedAt: now }, starter_garden: { ...garden, ...clean.starter_garden, updatedAt: now } }; store.put(result.starter_house_interior); store.put(result.starter_garden); await transactionDone(tx); return result; }
