...
export const DeveloperPanel = (state: AppState, actions: { close: () => void; interior: () => void; garden: () => void; moveCody: (destination: LocationRef) => void; pausePartner: () => void; resumePartner: () => void; decidePartner: () => void; triggerEvent: () => void; resetEvent: () => void; reset: () => void }): HTMLElement => {
  ...
  const activity = state.partnerActivity; appendDetail(details, "eventId", state.unknownSprout.eventId); appendDetail(details, "status", state.unknownSprout.status); appendDetail(details, "stage", state.unknownSprout.stage); appendDetail(details, "path", state.unknownSprout.path ?? "null"); appendDetail(details, "choiceHistory", state.unknownSprout.choiceHistory.join(", ") || "[]"); appendDetail(details, "開始日", state.worldStartedOn); appendDetail(details, "今日", new Date().toLocaleDateString("sv-SE")); appendDetail(details, "自律行動", activity.enabled ? "有効" : "一時停止"); appendDetail(details, "phase", activity.phase); appendDetail(details, "現在の行動", activity.actionId ? ACTION_DEFINITIONS[activity.actionId].label : "なし"); appendDetail(details, "目的地", activity.destination ? `${activity.destination.mapId}:${activity.destination.locationId}` : "なし"); appendDetail(details, "直近5行動", activity.recentActionIds.join(", ") || "なし");
  const controls = document.createElement("div"); controls.className = "developer-actions"; controls.append(makeButton("室内を表示", actions.interior), makeButton("庭を表示", actions.garden), makeButton(state.unknownSprout.status === "locked" ? "知らない芽を発生させる" : "知らない芽を初期状態へ戻す", state.unknownSprout.status === "locked" ? actions.triggerEvent : actions.resetEvent), makeButton("全体を初期状態へ戻す", actions.reset));
